...
const ProjectPopup = ({ project, onClose, darkMode }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 min-h-screen">
            ...
            >
                <button
                    className={`absolute top-2 right-0 text-4xl font-bold transition duration-200 ${darkMode ? "text-gray-300 hover:text-red-400" : "text-gray-500 hover:text-red-500"
                        }`}
                    onClick={onClose}
                >
                    &times;
...
const Projects = () => {
    ...
    return (
        <div
            className={`min-h-screen animate-fade-in p-10 transition-colors duration-300 ${darkMode ? "bg-gradient-to-r from-gray-900 to-gray-800 text-gray-100" : "bg-gradient-to-r from-slate-100 to-slate-200 text-blue-800"
                }`}
        >
            <h1
                className={`text-4xl font-bold mb-8 text-center ${darkMode ? "text-slate-400" : "text-blue-600"
                    }`}
            >
                My Projects
            </h1>
            ...
            >
                {projectData.map((project, index) => (
                    <SwiperSlide key={index}>
                        <div
                            className={`cursor-pointer p-4 rounded-lg shadow-md transform transition-transform duration-300 hover:scale-105 ${darkMode
                        ...
                        >
                            ...
                            <h3
                                className={`text-lg font-semibold ${darkMode ? "text-slate-400" : "text-blue-800"
                                    }`}
                            >
                                {project.name}
                            </h3>
